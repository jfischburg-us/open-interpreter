...
class Interpreter:
    ...
    def respond(self):
        ...
        if self.local:
            messages = tt.trim(
                self.messages, max_tokens=1048, system_message=system_message
            )
        else:
        ...
        if not self.local:
                ...
                try:
                    if self.use_azure:
                        try:
                            ...
                            print(f"Error: {e}")
                            print("Current message causing issue:")
                            print(self.messages[-1])
                        except Exception as e:
                            print("Unknown error making API call:")
                            traceback.print_exc()
                    else:
                        try:
                            print("Sending messages:", self.messages)
            ...
            response = self.llama_instance(
                prompt, stream=True, temperature=self.temperature, stop=["</s>"]
            )
        ...
        for chunk in response:
                    ...
                    if "content" in self.messages[-1]:
                        ...
                        if "```" in content:
                            ...
                            if (
                                ...
                                language = (
                                    lines[0].strip() if lines[0] != "" else "python"
                                )
            ...
            if chunk["choices"][0]["finish_reason"] or llama_function_call_finished:
                if (
                    ...
                    if (
                        not self.local
                        and "parsed_arguments" not in self.messages[-1]["function_call"]
                    ):
