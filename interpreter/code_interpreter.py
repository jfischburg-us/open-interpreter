...
code interpretation in various languages.
It allows displaying and running code blocks,
providing real-time output.

The module includes a 'CodeInterpreter' class and
related functions for managing code execution,
language-specific configurations, and more.
"""
...
def run_html(html_content):
    """
    Runs an HTML code snippet by creating a temporary HTML file with the
    content and opening it in the default web browser.
    Args:
        html_content (str): The HTML content to run.

    Returns:
        str: A message indicating the file's location and that it has
        been opened with the user's default web browser.
    """
    ...
    webbrowser.open("file://" + os.path.realpath(temp_html.name))

    return (
        f"Saved to {os.path.realpath(temp_html.name)} :"
        "and opened with the user's default web browser."
    )
...
language_map = {
    ...
    "applescript": {
        ...
        # (We'll prepend "osascript -e" every time,
        # not once at the start, so we want an empty shell)
...
class CodeInterpreter:
    ...
    They can control code blocks on the terminal, then be executed
    to produce an output which will be displayed in real-time.

    Args:
        language (str): The programming language to interpret the code.
        debug_mode (bool): Whether to enable debug mode for additional output.

    Attributes:
        language (str): The programming language being interpreted.
        proc (subprocess.Popen): The subprocess used for code execution.
        active_line (int): The active line number during code execution.
        debug_mode (bool): Indicates whether debug mode is enabled.
    """
    ...
    def start_process(self):
        """
        Starts the subprocess for code execution based
        on the selected language's start command.
        """
    ...
    def update_active_block(self):
        """
        Updates the active code block with the
        current active line and output.
        """
    ...
    """
        ...
        if self.language == "python":
            # This lets us stop execution when error happens
            # (which is not default -i behavior)
            # And solves a bunch of indentation problems--
            # if everything's indented, -i treats it as one block
        ...
        if self.print_cmd and self.language != "applescript":  # Applescript is special.
            # Needs it to be a shell command because 'return'
            # (very common) will actually return, halt script
    ...
    def add_active_line_prints(self, code):
        ...
        3) It really struggles with multiline stuff,
        so I've disabled that (but we really should fix and restore).
        """
        ...
        for i, line in enumerate(code_lines):
            ...
            # Iterate over the remaining lines to find the
            # leading whitespace of the next non-empty line
    ...
    def save_and_display_stream(self, stream, is_error_stream):
        """
        Handle each line of output and update the display.

        Args:
            stream: The output stream to read from.
            is_error_stream (bool): Indicates whether the stream is an error stream.
        """
...
def truncate_output(data):
    """
    Truncates the output data to a specified maximum length,
    adding a message if necessary.

    Args:
        data (str): The output data to truncate.

    Returns:
        str: The truncated output data with an optional message.
    """
...
def wrap_in_try_except(code):
    """
    Wrap the given code in a try-except block to catch exceptions.

    Args:
        code (str): The code to wrap.
