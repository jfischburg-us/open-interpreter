...
def run_html(html_content):
		try:
				...
				error_message = f"Failed to create or open HTML file: {str(e)}"
...
class CodeInterpreter:
		...
		def start_process(self):
				...
				try:
						...
						error_message = f"Failed to start subprocess for {self.language}: {str(e)}"
						traceback_string = traceback.format_exc()
						raise RuntimeError(f"{error_message}\n{traceback_string}")
...
def add_active_line_prints(self, code):
		...
		try:
				...
				error_message = f"Failed to add active line prints: {str(e)}"
				traceback_string = traceback.format_exc()
				raise RuntimeError(f"{error_message}\n{traceback_string}")
						...
						try:
								...
								for line in iter(stream.readline, ""):
										...
										if line.startswith("ACTIVE_LINE:"):
												...
												self.proc.terminate()
												raise KeyboardInterrupt("Execution was interrupted by user.")
										else:
												self.output += "\n" + line
								...
								error_message = f"Failed to read or handle stream: {str(e)}"
								traceback_string = traceback.format_exc()
								raise RuntimeError(f"{error_message}\n{traceback_string}")
...
def wrap_in_try_except(code):
		try:
				...
				error_message = f"Failed to wrap code in try-except block: {str(e)}"
				traceback_string = traceback.format_exc()
				raise RuntimeError(f"{error_message}\n{traceback_string}")
