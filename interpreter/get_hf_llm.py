...
First of all, THANK YOU. Open Interpreter is ALIVE,
ALL OVER THE WORLD because of YOU.
While this project is rapidly growing,
I've decided it's best for us to allow some technical debt.
The code here has duplication. It has imports in weird places.
It has been spaghettified to add features more quickly.
In my opinion **this is critical**
to keep up with the pace of demand for this project.
At the same time, I plan on pushing a significant
re-factor of `interpreter.py` and `code_interpreter.py` ~ September 11th.
After the re-factor, Open Interpreter's source code will be much simpler,
and much more fun to dive into.
Especially if you have ideas and **EXCITEMENT** about
the future of this project,
chat with me on discord: https://discord.gg/6p3fD6rBVm
- killian
"""
...
def get_hf_llm(repo_id, debug_mode, context_window):
    """
    Fetches and initializes a Code-Llama language model.

    :param repo_id: Repository ID on Hugging Face Model Hub.
    :param debug_mode: Flag to enable debug mode.
    :param context_window: Number of tokens in the context window.
    :return: Initialized Code-Llama language model instance or None.
    """
    if "TheBloke/CodeLlama-" not in repo_id:
        # ^ This means it was prob through the old --local,
        # so we have already displayed this message.
        # Hacky. Not happy with this
        print('', Markdown(
            f"**Open Interpreter** will use `{repo_id}` "
            "for local execution. Use your arrow keys to set up the model."),
            '')
    ...
    # First we give them a simple small medium large option.
    # If they want to see more, they can.

    if len(combined_models) > 3:

        # Display Small Medium Large options to user
        choices = [
            format_quality_choice(
                combined_models[0],
                "Small"),
            format_quality_choice(
                combined_models[
                    len(combined_models) // 2], "Medium"),
            format_quality_choice(
                combined_models[-1],
                "Large"),
            "See More"
        ...
        questions = [inquirer.List(
            'selected_model',
            message="Quality (smaller is faster, larger is more capable)",
            choices=choices)]
    ...
    if confirm_action(
        "Use GPU? (Large models might crash on GPU, "
            "but will run more quickly)"):
        n_gpu_layers = -1
    else:
        n_gpu_layers = 0
    ...
    for directory in directories_to_check:
        ...
        print(
            f"This language model was not found on your system.\n\n"
            f"Download to `{default_path}`?", "")
                ...
                for split_file in split_files:
                    ...
                    if os.path.exists(split_path):
                        if not confirm_action(
                            f"Split file {split_path} already exists. "
                                "Download again?"):
                            continue
                    hf_hub_download(
                        repo_id=repo_id,
    ...
    try:
        ...
        message = (
            "Local LLM interface package not found. "
            "Install `llama-cpp-python`?"
        )
        if confirm_action(message):
            # We're going to build llama-cpp-python
            # correctly for the system we're on
            ...
            def install_llama(backend):
                ...
                except subprocess.CalledProcessError as install_llama_error:
                    print(
                        f"Error during installation with "
                        f"{backend}: {install_llama_error}"
                        )
            ...
            print(
                '',
                Markdown("Finished downloading `Code-Llama` interface."),
                '')
            ...
            if platform.system() == "Darwin":
                ...
                if platform.machine() != "arm64":
                    print(
                        "Warning: You are using Apple Silicon (M1/M2) Mac "
                        "but your Python is not of 'arm64' architecture.")
                    print(
                        "The llama.ccp x86 version will be 10x "
                        "slower on Apple Silicon (M1/M2) Mac.")
                    print(
                        "\nTo install the correct version of "
                        "Python that supports 'arm64' architecture:")
                    print(
                        "1. Download Miniforge for M1/M2:")
                    print(
                        "wget "
...
def confirm_action(message):
    """
    Asks for user confirmation with a given message.
...
def list_gguf_files(repo_id: str) -> List[Dict[str, Union[str, float]]]:
    """
    Fetch all files from a given repository on
    Hugging Face Model Hub that contain 'gguf'.

    :param repo_id: Repository ID on Hugging Face Model Hub.
    :return: A list of dictionaries, each dictionary containing filename,
    size, and RAM usage of a model.
    """

    try:
        files_info = list_files_info(repo_id=repo_id)
    except Exception as gguf_err:
        if "authentication" in str(gguf_err).lower():
            print(
                "You likely need to be logged in to "
                "HuggingFace to access this language model.")
            print(
                f"Visit this URL to log in and apply for access to "
                f"this language model: https://huggingface.co/{repo_id}")
...
def group_and_combine_splits(
    models: List[
        Dict[
            str, Union[
                str, float]]]) -> List[Dict[str, Union[str, float]]]:
    """
    Groups filenames based on their base names and
    combines the sizes and RAM requirements.
...
def format_quality_choice(
    ...
    if name_override:
        ...
        model_size = (
            f"{name} | "
            f"Size: {model['Size']:.1f} GB, "
            f"Estimated RAM usage: {model['RAM']:.1f} GB"
            )
    return model_size
